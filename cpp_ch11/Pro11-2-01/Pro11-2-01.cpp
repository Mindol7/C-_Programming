...
ostream& operator<<(ostream& os, const Point& pos){ //연산자 오버로딩 (Point& pos는 Point객체를 매개값으로 받음)
    os<<pos.xpos<<" ,  "<<pos.ypos<<endl; //. 연산을 통해 객체의 멤버에 접근(직접 접근)
    return os;
}

ostream& operator<<(ostream& os, const Point* pos){ //(Point* pos는 Point객체를 가르키는 arr값 그자체를 인자로 받음)
    os<<(pos->xpos)<<" ,  "<<(pos->ypos)<<endl; //-> 연산을 통해 객체의 멤버에 접근
    return os; //주소값이 출력되는데 어떻게 값을 출력할 것인가?
}
